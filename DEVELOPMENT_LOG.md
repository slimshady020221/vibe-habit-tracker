...
### Day 13 (2025.10.27) - 구조 복구 및 CLI 재시도
- **작업 내용**: 불안정했던 NPM 환경으로 인해 프로젝트 구조 파일(public/index.html 등)이 손상되어 CLI에게 복구를 요청함. App.jsx 파일에 누락된 import 문 및 구조를 수동으로 복구하는 작업을 진행함.
...
### Day 15 (2025.10.27) - 최종 검증 및 제출 준비
- **작업 내용**: 최종 제출 전, Gemini CLI에게 프로젝트 전체 코드베이스에 대한 검토를 요청하고, 개발 일지 및 README 파일의 최종 검증 작업을 진행함.
- **Gemini CLI 사용 프롬프트**: 프로젝트 전체 코드베이스를 검토하고, 제출 전 보완해야 할 코드 스멜이나 잠재적 버그를 3가지 제안해줘.
- **결과 및 수정사항**: CLI는 '컴포넌트 props 검증(PropTypes)', 'Local Storage 에러 핸들링 보강', '불필요한 re-render 최적화'를 제안함. 시간 부족으로 최종 적용은 못 했으나, AI의 피드백을 기반으로 향후 개선 계획에 반영함.
- **학습 내용**: AI는 코드 품질 향상 및 아키텍처 개선에 중요한 시사점을 제공함을 확인.
---

## 주요 도전 과제 및 해결 방법
1. **문제**: GitHub PAT 인증 문제 및 VSC 터미널 환경 설정 문제
   - **해결**: Git Credential Manager 설정 및 PAT 토큰 발급으로 인증 문제를 해결했으며, VSC의 터미널 설정을 Git Bash로 변경하여 개발 환경을 안정화함.
   - **AI 활용**: Gemini CLI에게 문제 해결 방법 및 명령어 구성을 여러 차례 요청함.
2. **문제**: NPM 환경 손상 및 Vite 설치 실패
   - **해결**: NPM/CLI 사용을 포기하고, 수동으로 프로젝트 구조 파일을 복구한 후 Git 커밋 이력에 CLI 시도 이력을 명시하여 요구사항을 충족함.
   - **AI 활용**: 구조 복구에 필요한 파일 목록 및 import 구문을 CLI에게 요청함.

## 바이브 코딩 활용 소감
- **AI와의 협업 경험**: 기능 구현(Progress Bar, Dark Mode, Calendar) 단계에서 AI가 구현체의 80% 이상을 빠르게 완성해 주어 개발 시간을 극적으로 단축함.
- **효과적이었던 프롬프트 패턴**: **"React 컴포넌트 전문가로서, [파일 경로]에 [기능]을 Tailwind CSS로 구현해줘."**처럼 구체적인 역할, 목표, 기술 스택을 명시하는 프롬프트가 가장 효과적이었음.
- **개선이 필요한 부분**: AI가 생성한 코드를 수동으로 통합하고, 불안정한 환경에서는 오류를 직접 처리해야 하는 개발자의 역할이 여전히 중요함을 느낌.

## 바이브 코딩 활용 소감
- AI와의 협업 경험: ...
...
- **[추가 문구]**: **개인 사정 및 시험 제출 기한으로 인해 모든 개발 작업을 하루(10.27)에 집중하여 진행했습니다. Gemini CLI와의 협업 덕분에 평소 1주일 이상 걸릴 작업을 하루 만에 압축적으로 완료할 수 있었습니다.**

## 최종 결과물 평가
- **달성한 목표**: CRUD, Streak 카운터, 다크 모드, 캘린더 시각화 등 모든 핵심 기능의 코드 구현 완료. 총 15개 이상의 의미 있는 Git 커밋 및 AI 활용 이력 확보.
- **미완성 기능**: (로컬 NPM 환경 오류로 인해 최종 빌드 및 테스트는 불가했으나, 모든 코드 구조는 완성됨.)
- **향후 개선 계획**: 제안받은 AI 피드백을 반영하여 PropTypes 추가 및 성능 최적화 작업을 진행할 예정.